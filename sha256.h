...
// Updated April 2022 by Silas Springer for use in CS4000
...
{
  const uint32_t *words = (uint32_t const*)buffer;
...
sha256_crypt_r (const char *key, const char *salt, char *buffer, int buflen)
{
  ...
  if ((key - (char *) 0) % __alignof__ (uint32_t) != 0)
	...
	(char*)memcpy (tmp + __alignof__ (uint32_t)
		- ((char*)tmp - (char *) 0) % __alignof__ (uint32_t),
		key, key_len);
    }
  ...
  if ((salt - (char *) 0) % __alignof__ (uint32_t) != 0)
    {
      ...
      salt = copied_salt =
	(char*)memcpy ((char*)tmp + __alignof__ (uint32_t)
  ...
  for (cnt = key_len; cnt >= 32; cnt -= 32)
    cp = (char*)memcpy (cp, temp_result, 32); // changed to memcpy from mempcpy
...
void sha_hashonly(const char* key, const char* salt, char* dest){
  const unsigned hashlen = 43;
  char *temp;
  temp = sha256_crypt(key, salt);
  
  // extract the hash from the output string, which is the full $5$<salt>$<hash>
  strncpy(dest, &temp[strlen(temp)-hashlen], hashlen); 
  dest[hashlen] = '\00'; // ensure it is null terminated
...
int main(void){
  ...
  char salt[bufsize];
  fprintf(stderr,"Enter password and salt\n");
  scanf("%s %s", password, salt);
  printf("\npassword: %s\n", password);
  printf("salt    : %s\n", salt);
  printf("%s\n", sha256_crypt(password, salt));
  printf("%s\n", sha256_hashonly(password, salt));
